...
bool SemanticAnalyzer::isPotentialArgument(const SToken& arg) const {
    // Check if token could be a valid argument for current command without modifying state
    auto cmdIt = validCmd.find(currentCmd);
    if (cmdIt == validCmd.end()) return false;
    const auto& argsMap = cmdIt->second;
    return argsMap.find(arg) != argsMap.end();
}
